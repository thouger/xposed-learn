...
JValue InvokeXposedHandleHookedMethod(ScopedObjectAccessAlreadyRunnable& soa, const char* shorty,
  ...
  // ----------下面调用XposedBridge.handleHookedMethod()函数----------

  // 通过调用GetEntryPointFromJniPtrSize()函数获取Xposed在设置函数hook时保存在entry_point_from_jni_中的XposedHookInfo对象信息
  ...
  */
  // 这里相当于调用XposedBridge类的handleHookedMethod方法
  jobject result =
